sap.ui.jsview("views.Seller.CommissionRate", {

	/** Specifies the Controller belonging to this View. 
	* In the case that it is not implemented, or that "null" is returned, this View does not have a Controller.
	* @memberOf views.Seller.CommissionRate
	*/ 
	getControllerName : function() {
		return "views.Seller.CommissionRate";
	},
	...
	*/ 
	createContent : function(oController) {
		...
		var oFields = [{row1: [{type:"Label",value:"Date Requested",align: ""}
							  ,{type:"Text",value:"{/tranDetail/DateReq}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Agent ID",align: ""}
					  ,{type:"Text",value:"{/tranDetail/AgentCode}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Agent Name",align: ""}
					  ,{type:"Text",value:"{/tranDetail/AgentName}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Position",align: ""}
					  ,{type:"Text",value:"{/tranDetail/Position}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Sales Group",align: ""}
					  ,{type:"Text",value:"{/tranDetail/SalesGrp}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Brand",align: ""}
					  ,{type:"Text",value:"{/tranDetail/Brand}",align: ""}
					  ]}
					 ,{row1: [{type:"Label",value:"Contract Effectivity Date",align: ""}
					  ,{type:"Text",value:"{/tranDetail/EffectDate}",align: ""}
					  ]}
					 ];
		
		var oGenDetails = formDynamic(oFields, [], [ '110px', '200px', '50%' ]);

		//***************************** ROUTING MATRIX ******************************************/
		var tblItems = new sap.ui.table.Table({
		...
		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "Project (Building land)", textAlign: "Center"}),
		...
		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "Unit Type(Regular/Parking)", textAlign: "Center"}),
		...
		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "Old Commission Rate", textAlign: "Center"}),
        	template: new sap.ui.commons.TextView({textAlign: "End"}).bindProperty("text", "OldComrate"),
        	editable: false,
        	width: "15%"
        }));

		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "New Commission Rate", textAlign: "Center"}),
        	template: new sap.ui.commons.TextView({textAlign: "End"}).bindProperty("text", "NewComrate"),
        	editable: false,
        	width: "15%"
        }));

		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "Valid From Date", textAlign: "Center"}),
		...
		tblItems.addColumn(new sap.ui.table.Column({
        	label: new sap.ui.commons.Label({text: "Valid To Date", textAlign: "Center"}),
        	template: new sap.ui.commons.TextView().bindProperty("text",{
                parts: ["ValidTo" ],
                formatter: function(odate) {
        			return sapDateFormat(odate);
		...
		tblItems.setModel(this.getModel());
		tblItems.bindRows("/tranDetail/cr2DetailsSet");
		...
		var oTRemarks = new sap.ui.commons.TextArea({
			tooltip : "Remarks",
			value : '{/tranDetail/cr4RemarksSet}',
			width : "100%",
			rows : 5,
			enabled: false
		});
		...
		tblApprovers.addColumn(new sap.ui.table.Column({
        	...
        	template: new sap.ui.commons.TextView().bindProperty("text", "cr5CommentsSet").addStyleClass("sapUiTv1"),
        	editable: false
        }));
		
		tblApprovers.setModel(this.getModel());
		tblApprovers.bindRows("/tranDetail/cr5ApproversSet"); 
		//************************ APPROVAL PATH*******************************************/
        ...
        oSection3.setTitle('Commission Rate');		
        oSection3.setTooltip('Commission Rate');
        oSection3.addContent( tblItems);
		...
		var oBtnApprove = new sap.ui.commons.Button(sessionCache["pageID"] + "oBtnApprove",{
			tooltip : "Approve Change in Commission Rate",
			width : "100px",
			height : "30px",
			text : "Approve",
			press : function() {
				sap.ui.commons.MessageBox.show(
					      "Are you sure you want to approve request for Change in Commission Rate?",
		...
		var oBtnDisapprove = new sap.ui.commons.Button(sessionCache["pageID"] + "oBtnDisapprove",{
			tooltip : "Disapprove Change in Commission Rate",
			width : "100px",
			height : "30px",
			text : "Disapprove",
			press : function() {
				sap.ui.commons.MessageBox.show(
					      "Are you sure you want to disapprove request for Change in Commission Rate",
					      sap.ui.commons.MessageBox.Icon.INFORMATION,
					      "Disapprove Request",
					      [sap.ui.commons.MessageBox.Action.YES, sap.ui.commons.MessageBox.Action.NO],
					      function(bResult) { 
