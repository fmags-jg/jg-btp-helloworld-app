sap.ui.controller("views.Buyer.AccntAdjSpecial", {
...
//	}
	...
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Account Adjustment Special request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("ADJSPECIAL-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("ADJSPECIAL-"+oAction.toUpperCase(), "SUCCESS","",oDetail.Refnumber);
							...
							oBusyDialog.close();
							showActionMsg("ADJSPECIAL-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("ADJSPECIAL-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Account Adjustment Special Request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("ADJSPECIAL-"+oAction.toUpperCase(), "ERROR", oAction + " Account Adjustment Special Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
