sap.ui.controller("views.Buyer.ContractCancellation", {
...
//	}
	...
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Contract Cancellation request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("CCANCEL-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("CCANCEL-"+oAction.toUpperCase(), "SUCCESS","",oDetail.Refnumber);
							...
							oBusyDialog.close();
							showActionMsg("CCANCEL-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("CCANCEL-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Contract Cancellation Request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("CCANCEL-"+oAction.toUpperCase(), "ERROR", oAction + " Contract Cancellation Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
