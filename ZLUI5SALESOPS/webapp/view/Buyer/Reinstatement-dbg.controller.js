sap.ui.controller("views.Buyer.Reinstatement", {
...
//	}
	...
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Reinstatement request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("REINST-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("REINST-"+oAction.toUpperCase(), "SUCCESS","",oDetail.Refnumber);
							...
							oBusyDialog.close();
							showActionMsg("REINST-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("REINST-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Reinstatement Request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("REINST-"+oAction.toUpperCase(), "ERROR", oAction + " Reinstatement Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
