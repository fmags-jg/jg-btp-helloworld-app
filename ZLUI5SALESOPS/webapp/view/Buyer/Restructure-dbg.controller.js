sap.ui.controller("views.Buyer.Restructure", {
...
//	}
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Restructure request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("RESTRUC-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("RESTRUC-"+oAction.toUpperCase(), "SUCCESS","",oDetail.Refnumber);
							...
							oBusyDialog.close();
							showActionMsg("RESTRUC-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("RESTRUC-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Restructure Request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("RESTRUC-"+oAction.toUpperCase(), "ERROR", oAction + " Restructure Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
