sap.ui.controller("views.Buyer.EarlyMoveIn", {
...
//	}
	...
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Early Move-In request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("EMI-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("EMI-"+oAction.toUpperCase(), "SUCCESS","",oDetail.RefNumber);
							...
							oBusyDialog.close();
							showActionMsg("EMI-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("EMI-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Early Move-In Request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("EMI-"+oAction.toUpperCase(), "ERROR", oAction + " Early Move-In Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
