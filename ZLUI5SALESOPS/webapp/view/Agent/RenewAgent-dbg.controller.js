sap.ui.controller("views.Agent.RenewAgent", {
...
//	}
	...
	approveDisapprove: function(oAction,oRemarks) {
		var oBusyDialog = new sap.m.BusyDialog({text: oAction + ' Agent Contract Renewal request. Please wait...'});
		oBusyDialog.open();
				...
				$.ajax({
					...
					success : function(data,response,xhrObj) {
						try {
							if(data.d.Type != "S") {
								oBusyDialog.close();
								showActionMsg("RENEWAGENT-"+oAction.toUpperCase(), "ERROR",data.d.Message);
								return false;
							} 
							
							showActionMsg("RENEWAGENT-"+oAction.toUpperCase(), "SUCCESS","",oDetail.Refnumber);
							...
							oBusyDialog.close();
							showActionMsg("RENEWAGENT-"+oAction.toUpperCase(), "ERROR", ex.message);
		                }
					},
					error : function(err) {
						oBusyDialog.close();
						showActionMsg("RENEWAGENT-"+oAction.toUpperCase(), "ERROR", "Unable to " + oAction + " Agent Contract Renewal request.\nStatus Code: " + err.status + "\nStatus Text: " + err.statusText + "\nDescription: " + err.responseText);
					}
				});
			},
			error : function() {
				oBusyDialog.close();
				showActionMsg("RENEWAGENT-"+oAction.toUpperCase(), "ERROR", oAction + " Agent Contract Renewal Request. Unable to fecth security token.");
			}
		});
		return ret;
	},
